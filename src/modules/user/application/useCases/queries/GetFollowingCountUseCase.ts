...
export interface GetFollowingCountQuery {
  userId: string; // DID or handle
}

export interface GetFollowingCountResult {
  count: number;
}
...
export class GetFollowingCountUseCase implements UseCase<
  GetFollowingCountQuery,
  Result<GetFollowingCountResult>
> {
  constructor(
  ...
  async execute(
    query: GetFollowingCountQuery,
  ): Promise<Result<GetFollowingCountResult>> {
    ...
    try {
      // Get count of users that this user follows
      ...
      if (countResult.isErr()) {
        return err(
          new Error(
            `Failed to retrieve following count: ${countResult.error instanceof Error ? countResult.error.message : 'Unknown error'}`,
          ),
        );
      }
      ...
      return err(
        new Error(
          `Failed to retrieve following count: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ),
      );
    }
  }

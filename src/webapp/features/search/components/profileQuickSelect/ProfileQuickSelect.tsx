...
export default function ProfileQuickSelect() {
  ...
  const {
    ...
    queryKey: ['bluesky user search quick select', debounced],
  ...
  return (
    <Combobox
      ...
      onOptionSubmit={(handleValue) => {
        ...
        if (selectedActor) {
          router.push(`/profile/${selectedActor.handle}`);
          setInputValue('');
    ...
    >
      <Combobox.Target>
        <TextInput
          placeholder="Quick select profile..."
          value={inputValue}
          onChange={(e) => {
      ...
      <Combobox.Dropdown
        hidden={debounced.trim().length === 0}
      >
        <Combobox.Options>
